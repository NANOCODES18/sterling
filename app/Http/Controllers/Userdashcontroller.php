...
use App\Models\TradeSubscription;
...
class Userdashcontroller extends Controller
{
    ...
    public function __construct()
    {
        ...
        // Apply kyc.verified middleware to all methods except the KYC pages
        $this->middleware('kyc.verified')->except(['showKycForm', 'submitKyc', 'showKycStatus']);
    }
    ...
    public function dash_copytrades(Request $req)
    {
        dd('still under construction');
    }
    ...
    public function dashb_depositsubmit(Request $req)
    {
        ...
        if ($result) {
            ...
            // return redirect()->route('dashb_deposits')->with('success', $message)->with($data);
            return back()->with('sendaddress', $methacc)->with('success', $message);
        } else {
            # code...
    ...
    // Function to get the list of traders and display on traders.blade.php
    public function showTraders()
    {
        // Fetch all traders from the database
        $traders = Trader::all();

        // Pass the list of traders to the traders.blade.php view
        return view('dashb.dashb_traders', ['traders' => $traders]);
    }

    // Function to get the trades of a selected trader
    // public function showTrades($traderId)
    // {
    //     // Fetch the selected trader's trades from the database
    //     $trades = Trade::where('trader_id', $traderId)->get();

    //     // Pass the trades to the trades.blade.php view
    //     return view('dashb.dashb_trades', ['trades' => $trades]);
    // }

    // Function to get the trades of a selected trader with his name in the collection
    //     public function showTrades($traderId)
    // {
    //     // Fetch the selected trader's trades along with the trader's name
    //     $trades = Trade::with('trader')->where('trader_id', $traderId)->get();

    //     // Pass the trades to the trades.blade.php view
    //     return view('dashb.dashb_trades', ['trades' => $trades]);
    // }

    // Function to get the trades of a selected trader and his name
    public function showTrades($traderId)
    {
        // Fetch the selected trader's trades from the database, including the new fields
        $trades = Trade::where('trader_id', $traderId)->get(['id', 'minimum', 'maximum', 'average_percentage', 'fees', 'investors', 'nickname']); // Select new fields

        // Fetch the trader's name using the traderId
        $traderName = Trader::find($traderId)->name; // Assuming trader exists

        // Pass the trades and trader name to the trades.blade.php view
        return view('dashb.dashb_trades', [
            'trades' => $trades,
    ...
    // Function to process the copy trade and deduct balance
    public function copyTrade(Request $req, $tradeId)
    {
        // Get the authenticated user (the one copying the trade)
        $user = auth()->user();
        ...
        // Get the user's input amount (from form)
        $amount = $req->amount;
        ...
        // Retrieve the voucher code from the request
        ...
        if (!empty($voucherCode)) {
            ...
            if (!$voucher) {
                return redirect()->back()->withErrors(['voucher_code' => 'Voucher not found crosscheck the voucher code or proceed without voucher.']);
            }

            // Check if the voucher is active
            if ($voucher->status !== 'active') {
                return redirect()->back()->withErrors(['voucher_code' => 'This voucher is either used or inactive.']);
            }

            // Store the voucher amount in a variable
        ...
        // Validate that the amount is within the min and max trade values
        if ($amount < $trade->minimum || $amount > $trade->maximum) {
            return back()->with('error', 'Amount must be between the minimum and maximum trade values.');
        }

        // Ensure the user has enough balance
        if ($user_funds->balance + $voucherAmount < $amount) {
            return back()->with('error', 'Insufficient balance to copy this trade.');
        }

        // Deduct the amount from the user's balance
        $amount = $amount - $voucherAmount;
        $user_funds->balance -= $amount;
        $user_funds->save();

        // Check if the user is already subscribed to the trade in the TradeSubscription table
        $isSubscribed = TradeSubscription::where('user_id', $user->id)
            ->where('trade_id', $tradeId)
            ->exists();

        // If not subscribed, create a new subscription for the user
        if (!$isSubscribed) {
            TradeSubscription::create([
                'user_id' => $user->id,
                'trade_id' => $tradeId,
                'amount' => $amount,
                // Add other necessary fields (e.g., subscription date, expiration date)
            ]);
        }
        ...
        if ($amount > $plan_from_db->maximum) {
            ...
            if ($amount > $user_fund->balance + $voucherAmount) {
                # code...
                return back()->with('error', 'Account balance low for the amount, please try a lower amount or fund your account');
            } else {
                # code...
                ...
                if ($result) {
                    ...
                    $user = User::where("id", $this->logged_in_user()->id)->first();
                    $email = $user->email;
                    $mail = "This is to notify you that your request to copy trades have been aproved and is starting shortly";
                    $mailtitle = "Trade Copying Notification";
                    $emaildata = ['data' => $email, 'email_body' => $mail, 'email_header' => $mailtitle];

                    Mail::to($email)->send(new Adminmail($emaildata));
                    return back()->with("success", "the amount of $$amount is being used to copy the trades of the trader");
                } else {
                    # code...
                    return back()->with("error", "Subscribed but failed to copy trade please try again!");
                }
            }
        }
    ...
    // Show the KYC form to the user
    public function showKycForm()
    {
        // Check if the user already submitted KYC
        $kyc = KycSubmission::where('user_id', Auth::id())->first();

        // If KYC is already submitted, redirect to status page
        if ($kyc) {
            return redirect()->route('kyc.status');
        }

        // Show the KYC submission form if no KYC has been submitted
        return view('dashb.dahsb_kyc');
    }
    ...
    public function submitKYC(Request $request)
    {
        ...
        $request->validate([
            ...
            'id_document' => 'required|file|mimes:jpg,jpeg,png,pdf|max:2048', // Adjust max size as needed
        ]);

        // Store the uploaded document in the 'kyc_documents' directory
        $filePath = $request->file('id_document')->store('kyc_documents', 'public');

        // Check if the user already has a KYC submission
        $existingSubmission = KycSubmission::where('user_id', auth()->id())->first();

        if ($existingSubmission) {
            // Update the existing KYC submission
            $existingSubmission->update([
                'full_name' => $request->input('full_name'),
                'dob' => $request->input('dob'),
                'address' => $request->input('address'),
                'id_document_path' => $filePath,
                'status' => 'pending', // Reset status to pending if resubmitted
            ]);

            return back()->with('success', 'KYC submission updated successfully!');
        }
        ...
        return back()->with('success', 'KYC submitted successfully! We will review it soon.');
    }


    // Show the user's KYC status

    public function showKycStatus()
    {
        // Fetch the authenticated user's KYC submission from the database
        $kyc = KycSubmission::where('user_id', Auth::id())->first();

        // If the user hasn't submitted KYC, redirect to the KYC form
        if (!$kyc) {
            return redirect()->route('kyc.form')->with('error', 'Please submit your KYC.');
        }

        // Show the KYC status page with the fetched KYC submission data
        return view('dashb.dashb_status', compact('kyc'));
    }



    private function sendKYCDataToAPI($data, $filePath)
    {
        // Make a request to the KYC provider's API with the user's data
        // Example using cURL or any HTTP client like Guzzle
        // Send $data and $filePath to the KYC API
    ...
    public function getSubscribedTrades()
    {
        // Get the authenticated user
        $user = auth()->user();

        // Fetch all trade subscriptions for the user
        $subscriptions = TradeSubscription::where('user_id', $user->id)->with('trade')->get();

        // Return the list of subscribed trades
        return view('dashb.dashb_subscribed_trades', ['subscribedTrades' => $subscriptions]);
    }
    ...
    public function showUploadForm()
    {
        $data = [];
        $data['title'] = "Market Charts";
        return view('dashb.dashb_deposit_proof_upload',$data);
    }

    public function store(Request $request)
    {
        $request->validate([
            'file' => 'required|mimes:jpeg,png,pdf|max:2048', // Only images or PDFs, max size 2MB
        ...
        $filePath = $request->file('file')->store('proof_of_payments', 'public');
