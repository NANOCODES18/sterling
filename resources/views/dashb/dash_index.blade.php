...
@section('body')
                    ...
                    <div class="card-body d-flex flex-column">
                        ...
                        <div class="d-flex flex-row mt-4 mb-4">
                            ...
                            <div class="wrapper ml-4 border-left pl-4">
                                ...
                                <div class="d-flex align-items-center">
                                    $<h5 class="font-weight-medium mb-0" id="balance">{{$funds->totalprofit}}</h5>
                            ...
                            <div class="row">
                                <div class="col-6 col-md-12">
                                    <form class="forms-sample mt-2" action='ref.html' method="post">
                                        <a href="mailto:?subject=Invitation&body=Hello+dear%2C+I+am+happy+to+invite+you+to+join+sterling+%2C+a+reliable+investment+website+that+helps+you+to+trade+and+return+your+profit.+Use+my+link+to+register+{{route('register')}}%3Frefid%3D{{Auth::user()->id}}"
                                ...
                                <div class="col-6 col-md-12">
                                    <form class="forms-sample mt-2" action='ref.html' method="post">
                                        <a href="https://wa.me/?text=Hello+dear%2C+I+am+happy+to+invite+you+to+join+sterling+%2C+a+reliable+investment+website+that+helps+you+to+trade+and+return+your+profit.+Use+my+link+to+register+{{route('register')}}%3Frefid%3D{{Auth::user()->id}}"
                                ...
                                <div class="col-6 col-md-12">
                                    <form class="forms-sample mt-2" action='ref.html' method="post">
                                        <a href="https://t.me/share/?text=Hello+dear%2C+I+am+happy+to+invite+you+to+join+sterling+%2C+a+reliable+investment+website+that+helps+you+to+trade+and+return+your+profit.+Use+my+link+to+register+{{route('register')}}%3Frefid%3D{{Auth::user()->id}}"
    ...
    {{-- <script>
        let baseBalance = parseFloat(document.getElementById('balance').innerText);
        let currentBalance = baseBalance;
        let phase = 'up'; // Can be 'up' or 'down'
        ...
        function updateBalance() {
          ...
          // Define min and max limits for the balance fluctuation
          let min = baseBalance * 0.7;  // 30% below base
          let max = baseBalance * 1.2;  // 20% above base
        
          // Determine the amount by which to adjust the balance in each step
          let fluctuation = (Math.random() * 0.02 * baseBalance); // Up to 2% increment/decrement per step
        
          // Adjust the balance based on the current phase
          if (phase === 'up') {
            currentBalance += fluctuation; // Increase balance
            if (currentBalance > max) {
              currentBalance = max; // Cap it at 20% above base value
            }
          } else {
            currentBalance -= fluctuation; // Decrease balance
            if (currentBalance < min) {
              currentBalance = min; // Cap it at 30% below base value
            }
          }
        ...
        // Function to randomly switch phases
        function switchPhase() {
          ...
          // Randomly switch between 'up' and 'down' phases
          phase = Math.random() > 0.5 ? 'up' : 'down';
        
          // Set a random duration between 3 to 10 seconds for the current phase
          let phaseDuration = Math.floor(Math.random() * 7000) + 3000;
        
          // Call this function again after the current phase duration
          setTimeout(switchPhase, phaseDuration);
        }
        
        // Start the fluctuation and phase switching if balance is not zero
        if (baseBalance !== 0) {
          setInterval(updateBalance, 1000); // Update balance every second
          switchPhase(); // Start the phase switching
        }
